...
static void disk_norm(int *bsize, int *dfree, int *dsize)
{
	...
	if (maxdisksize) {
		...
		if (*dfree > maxdisksize)
			*dfree = maxdisksize - 1; /* the -1 should stop
...
int disk_free(char *path, int *bsize, int *dfree, int *dsize)
{
	...
	if (df_command && *df_command) {
		...
		slprintf(outfile, sizeof(outfile) - 1, "%s/dfree.smb.%d",
		         tmpdir(), (int) getpid());
		...
		*/
		if (disk_quotas(path, &bsizeq, &dfreeq, &dsizeq)) {
			...
			dfree_retval = (dfree_retval < dfreeq_retval)
			                 ? dfree_retval
			                 : dfreeq_retval;
			/* maybe dfree and dfreeq are calculated using different
			   bsizes so convert dfree from bsize into bsizeq */
			/* avoid overflows due to multiplication, so do not:
			      *dfree = ((*dfree) * (*bsize)) / (bsizeq);
			   bsize and bsizeq are powers of 2 so its better to
			   to divide them getting a multiplication or division
			   factor for dfree. Rene Nieuwenhuizen (07-10-1997) */
			if (*bsize >= bsizeq)
				*dfree = *dfree * (*bsize / bsizeq);
			else
	...
	*/
	if (disk_quotas(path, &bsizeq, &dfreeq, &dsizeq)) {
		disk_norm(&bsizeq, &dfreeq, &dsizeq);
		dfreeq_retval = ((bsizeq) / 1024) * (dfreeq);
		dfree_retval = (dfree_retval < dfreeq_retval) ? dfree_retval
		                                              : dfreeq_retval;
...
address %x. Error was %s\n",
	...
	if (getsockname(oplock_sock, (struct sockaddr *) &sock_name, &len) <
	    0) {
		DEBUG(0, ("open_oplock_ipc: Failed to get local UDP port. "
		          "Error was %s\n",
		          strerror(errno)));
		close(oplock_sock);
		oplock_sock = -1;
...
static BOOL process_local_message(int sock, char *buffer, int buf_size)
{
	...
	DEBUG(5, ("process_local_message: Got a message of length %d from port "
	          "(%d)\n",
	          msg_len, from_port));
	...
	case OPLOCK_BREAK_CMD:
		...
		if (msg_len != OPLOCK_BREAK_MSG_LEN) {
			DEBUG(
			    0,
			    ("process_local_message: incorrect length for OPLOCK_BREAK_CMD (was %d, \
should be %d).\n",
		...
		{
			uint32 remotepid =
			    IVAL(msg_start, OPLOCK_BREAK_PID_OFFSET);
			uint32 dev = IVAL(msg_start, OPLOCK_BREAK_DEV_OFFSET);
			uint32 inode =
			    IVAL(msg_start, OPLOCK_BREAK_INODE_OFFSET);
			struct timeval tval;
			struct sockaddr_in toaddr;

			tval.tv_sec = IVAL(msg_start, OPLOCK_BREAK_SEC_OFFSET);
			tval.tv_usec =
			    IVAL(msg_start, OPLOCK_BREAK_USEC_OFFSET);

			DEBUG(
			    5,
			    ("process_local_message: oplock break request from \
...
not returning udp message.\n"));
				...
				DEBUG(
				    3,
				    ("process_local_message: oplock break requested with no outstanding \
oplocks. Returning success.\n"));
			}

			/* Send the message back after OR'ing in the 'REPLY'
			 * bit. */
			SSVAL(msg_start, UDP_MESSAGE_CMD_OFFSET,
			      OPLOCK_BREAK_CMD | CMD_REPLY);
			...
			if (sendto(sock, msg_start, OPLOCK_BREAK_MSG_LEN, 0,
			           ...
			           sizeof(toaddr)) < 0) {
				DEBUG(0, ("process_local_message: sendto "
				          "process %d failed. Errno was %s\n",
			...
			DEBUG(
			    5,
			    ("process_local_message: oplock break reply sent to \
...
reply - dumping info.\n"));
		...
		if (msg_len != OPLOCK_BREAK_MSG_LEN) {
			DEBUG(
			    0,
			    ("process_local_message: ubr: incorrect length for reply \
(was %d, should be %d).\n",
			     msg_len, OPLOCK_BREAK_MSG_LEN));
			return False;
		}

		{
			...
			DEBUG(
			    0,
			    ("process_local_message: unsolicited oplock break reply from \
pid %d, port %d, dev = %x, inode = %x\n",
			     remotepid, from_port, dev, inode));
		}
		return False;
	...
	default:
		DEBUG(0, ("process_local_message: unknown UDP message command "
		          "code (%x) - ignoring.\n",
                          ...
                          uint32 inode)
{
	...
	if (pid == share_entry->pid) {
		...
		if (share_entry->op_port != oplock_port) {
			DEBUG(
			    0,
			    ("request_oplock_break: corrupt share mode entry - pid = %d, port = %d \
should be %d\n",
	...
	SIVAL(op_break_msg, OPLOCK_BREAK_SEC_OFFSET,
	      (uint32) share_entry->time.tv_sec);
	SIVAL(op_break_msg, OPLOCK_BREAK_USEC_OFFSET,
	      (uint32) share_entry->time.tv_usec);
	...
	DEBUG(
	    3,
	    ("%s request_oplock_break: sending a oplock break message to pid %d on port %d \
for dev = %x, inode = %x\n",
	...
	if (sendto(oplock_sock, op_break_msg, OPLOCK_BREAK_MSG_LEN, 0,
		...
		DEBUG(
		    0,
		    ("%s request_oplock_break: failed when sending a oplock break message \
to pid %d on port %d for dev = %x, inode = %x. Error was %s\n",
		     timestring(), share_entry->pid, share_entry->op_port, dev,
	 ...
	 * return in (OPLOCK_BREAK_TIMEOUT + OPLOCK_BREAK_TIMEOUT_FUDGEFACTOR)
	 * seconds. While we get messages that aren't ours, loop.
	 */
	...
	while (time_left >= 0) {
		...
		if (receive_local_message(
		        oplock_sock, op_break_reply, sizeof(op_break_reply),
		        time_left ? time_left * 1000 : 1) == False) {
			if (smb_read_error == READ_TIMEOUT) {
				DEBUG(
				    0,
				    ("%s request_oplock_break: no response received to oplock break request to \
pid %d on port %d for dev = %x, inode = %x\n",
				     timestring(), share_entry->pid,
				     share_entry->op_port, dev, inode));
				/*
				 * This is a hack to make handling of failing
				 * clients more robust. If a oplock break
				 * response message is not received in the
				 * timeout period we may assume that the smbd
				 * servicing that client holding the oplock has
				 * died and the client changes were lost anyway,
				 * so we should continue to try and open the
				 * file.
				 */
				break;
			} else
				DEBUG(
				    0,
				    ("%s request_oplock_break: error in response received to oplock break request to \
pid %d on port %d for dev = %x, inode = %x. Error was (%s).\n",
		...
		if (reply_msg_len != OPLOCK_BREAK_MSG_LEN) {
			/* Ignore it. */
			DEBUG(0, ("%s request_oplock_break: invalid message "
			          "length received. Ignoring\n",
			          timestring()));
			continue;
		}
		...
		if ((SVAL(reply_msg_start, UDP_MESSAGE_CMD_OFFSET) &
		     CMD_REPLY) &&
			...
			process_local_message(oplock_sock, op_break_reply,
...
static void process(void)
{
	...
	while (True) {
		...
		for (counter = SMBD_SELECT_LOOP; !receive_message_or_smb(
		         Client, oplock_sock, InBuffer, BUFFER_SIZE,
		...
		if (got_smb)
			...
			process_local_message(oplock_sock, InBuffer,
			                      BUFFER_SIZE);
	}
}
