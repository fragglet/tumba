	...
	while (!found) {
	  ...
	  hash_name(fname);
			...
			*mode = dos_mode(cnum, path, sbuf.st_mode);
			...
			if (((*mode & ~dirtype) & (aHIDDEN | aSYSTEM | aDIR)) !=
			    0) {
...
BOOL open_sockets(BOOL daemon, int port)
{
	...
	if (!daemon &&
	    (getsockopt(0, SOL_SOCKET, SO_TYPE, (char *) &type, &optlen) ==
	     -1) &&
	    (errno == ENOTSOCK)) {
		Debug(0,
		      "standard input is not a socket, will create a socket\n");
		daemon = True;
	}
	...
	if (daemon) {
		...
		if (setsockopt(s, SOL_SOCKET, SO_REUSEADDR, &sockopt,
		               sizeof(int))) {
	...
	{
		int one = 1;
		setsockopt(Client, SOL_SOCKET, SO_KEEPALIVE, (char *) &one,
		           sizeof(one));
	}
                 ...
                 int line)
{
	...
	Debug(2, "%s error packet at line %d cmd=%d enum=%d\n", timestring(),
	      line, (int) CVAL(inbuf, smb_com), error_code);
...
int reply_tcon_and_X(char *inbuf, char *outbuf, int length, int bufsize)
{
	...
	if (smb_com2 != 0xFF)
		outsize +=
		    chain_reply(smb_com2, inbuf, inbuf + smb_off2 + 4, outbuf,
		                outbuf + outsize, length, bufsize);
...
int reply_search(char *inbuf, char *outbuf, int length, int bufsize)
{
	...
	if (status_len == 0) {
		...
		strncpy(Connections[cnum].dirpath, dptr_path(dptr_num),
		        sizeof(pstring));
		strlower(mask);
	}
	...
	if (become_user(cnum)) {
		...
		if (status_len == 0) {
			if (!start_dir(cnum, directory))
				...
				dptr_num = dptr_create(Connections[cnum].dirptr,
				                       directory);
		}
	...
	Debug(
	    2,
	    "%s search mask=%s directory=%s cnum=%d dirtype=%d numentries=%d\n",
...
int reply_open(char *inbuf, char *outbuf, int length, int bufsize)
{
  ...
  attribute = SVAL(inbuf,smb_vwv1);
	  ...
	  if (openmode != O_RDONLY) openmode |= O_CREAT;
			...
			if (Files[fnum].fd >= 0) {
				...
				if (fstat(Files[fnum].fd, &sbuf) == 0) {
					size = sbuf.st_size;
					fmode =
					    dos_mode(cnum, fname, sbuf.st_mode);
	...
	Debug(
	    2,
	    "%s open %s fd=%d fnum=%d cnum=%d mode=%d omode=%d (numopen=%d)\n",
...
int reply_open_and_X(char *inbuf, char *outbuf, int length, int bufsize)
{
  ...
  uint32 smb_time = IVAL(inbuf,smb_vwv6);
	...
	Debug(
	    2,
	    "%s openX %s fd=%d fnum=%d cnum=%d mode=%d omode=%d (numopen=%d)\n",
...
int reply_mknew(char *inbuf, char *outbuf, int length, int bufsize)
{
	...
	if (createmode & aVOLID) {
		Debug(0,
		      "Attempt to create file (%s) with volid set - please "
		      "report this\n",
		      fname);
	}
	...
	if (become_user(cnum)) {
		...
		if (fnum >= 0 && ok) {
			Files[fnum].fd =
			    open(fname, O_RDWR | O_CREAT | O_TRUNC, unixmode);
			if (Files[fnum].fd >= 0) {
				num_files_open++;
				Files[fnum].open = True;
	...
	Debug(2, "%s mknew %s fd=%d fnum=%d cnum=%d dmode=%d umode=%o\n",
	      timestring(), fname, Files[fnum].fd, fnum, cnum, createmode,
	      unixmode);
...
int reply_ctemp(char *inbuf, char *outbuf, int length, int bufsize)
{
	...
	Debug(2, "%s ctemp %s fd=%d fnum=%d cnum=%d dmode=%d umode=%o\n",
	      timestring(), fname2, Files[fnum].fd, fnum, cnum, createmode,
	      unixmode);
...
int reply_unlink(char *inbuf, char *outbuf, int length, int bufsize)
{
	...
	isrootdir = (strequal(directory, "./") || strequal(directory, "."));
...
int reply_readbraw(char *inbuf, char *outbuf, int length, int bufsize)
{
	...
	if (become_user(cnum)) {
		if (OPEN_FNUM(fnum) && (Files[fnum].cnum == cnum)) {
			...
			if (!Files[fnum].can_lock ||
				...
				nread = read_with_timeout(
				    Files[fnum].fd, &read_buf[4], mincount,
	...
	Debug(2,
	      "%s readbraw fnum=%d cnum=%d max=%d min=%d timeout=%d nread=%d\n",
	      timestring(), fnum, cnum, maxcount, mincount, timeout, nread);
	...
	while (nwritten < nread) {
		ret =
		    write_socket(Client, read_buf + nwritten, nread - nwritten);
		if (ret <= 0) {
			Debug(0,
...
int reply_lockread(char *inbuf, char *outbuf, int length, int bufsize)
{
	...
	if (become_user(cnum)) {
		if (OPEN_FNUM(fnum) && (Files[fnum].cnum == cnum)) {
			if (do_lock(fnum, cnum, numtoread, startpos, &eclass,
			            &ecode))
...
int reply_read_and_X(char *inbuf, char *outbuf, int length, int bufsize)
{
  ...
  uint32 smb_timeout = SVAL(inbuf,smb_vwv7);
	...
	if (become_user(cnum)) {
		if (OPEN_FNUM(smb_fid) && (Files[smb_fid].cnum == cnum)) {
			...
			if (!Files[smb_fid].can_lock ||
				...
				nread =
				    read(Files[smb_fid].fd, data, smb_maxcnt);
				ok = True;
			}
		}
		unbecome_user();
	...
	Debug(
	    2,
	    "%s readX fnum=%d cnum=%d min=%d max=%d nread=%d com2=%d off2=%d\n",
	    timestring(), smb_fid, cnum, smb_mincnt, smb_maxcnt, nread,
	    smb_com2, smb_off2);
	...
	if (become_user(cnum)) {
		if (OPEN_FNUM(fnum) && (Files[fnum].cnum == cnum)) {
			...
			if (!Files[fnum].can_lock ||
				...
				nwritten = write_with_timeout(
				    Files[fnum].fd, data, numtowrite, timeout);
				if (write_through)
					fsync(Files[fnum].fd);
				ok = True;
			}
			...
			Debug(2,
			      "%s writebraw message 1 fnum=%d cnum=%d num=%d "
			      "wrote=%d\n",
			...
			if (!read_data(Client, read_buf, 4)) {
				Debug(0, "Failed to read length of secondary "
				         "writebraw\n");
				close_sockets();
				exit(1);
			}
			...
			if (!read_data(Client, read_buf, numtowrite)) {
				Debug(0, "Failed to read data in secondary "
				         "writebraw\n");
				close_sockets();
				exit(1);
			}

			/* Set up outbuf to return the correct type of error
			 * message */
	...
	if (become_user(cnum)) {
		if (OPEN_FNUM(fnum) && (Files[fnum].cnum == cnum)) {
			...
			if (!Files[fnum].can_lock ||
				...
				/* X/Open SMB protocol says that, unlike
				   SMBwrite if the length is zero then NO
				   truncation is done, just a write of zero. To
				   truncate a file, use SMBwrite. */
				if (smb_dsize == 0)
					nwritten = 0;
				else
...
int reply_lock(char *inbuf, char *outbuf, int length, int bufsize)
{
	...
	Debug(2, "%s lock fd=%d fnum=%d cnum=%d ofs=%d cnt=%d\n", timestring(),
	      Files[fnum].fd, fnum, cnum, offset, count);
...
int reply_mv(char *inbuf, char *outbuf, int length, int bufsize)
{
	...
	if (become_user(cnum)) {
		if (reduce_name(oldname, HOME(cnum)) &&
		    reduce_name(newname, HOME(cnum)))
...
int reply_lockingX(char *inbuf, char *outbuf, int length, int bufsize)
{
  ...
  uint32 timeout = IVAL(inbuf,smb_vwv4);
	...
	for (i = 0; i < num_ulocks; i++) {
		if (do_unlock(fnum, cnum, SMB_LKLEN(data, i),
		              SMB_LKOFF(data, i), &eclass, &ecode))
			return ERROR(eclass, ecode);
	}
	...
	if (i != num_locks) {
		for (; i >= 0; i--) {
			do_unlock(fnum, cnum, SMB_LKLEN(data, i),
			          SMB_LKOFF(data, i), &dummy, &dummy);
		}
		return ERROR(eclass, ecode);
	}
                ...
                char *outbuf2, int size, int bufsize)
{
	...
	/* process the request */
	outsize =
	    switch_message(type, ibuf, obuf,
	                   (size + smb_wct) - (int) (inbuf2 - inbuf), bufsize);

	/* copy the new reply header over the old one, but preserve
	   the smb_com field */
	memcpy(outbuf + smb_com + 1, obuf + smb_com + 1,
	       smb_wct - (smb_com + 1));
...
void process(void)
{
	...
	int nread;
	int deadtime =
	    lp_deadtime() * 60 * 1000 * 1000; /* the deadtime is in seconds */
	...
	while (True) {
		...
		nread = construct_reply(InBuffer, OutBuffer, nread,
		                        MIN(BUFFER_SIZE, lp_maxxmit()));
		...
		if (nread != -1) {
			...
			if (nread != smb_len(OutBuffer) + 4) {
				Debug(0,
				      "Invalid message response size! %d %d\n",
		...
		if (type == SMBtdis) {
			...
			if (i != MAX_CONNECTIONS)
				continue;
			/* There are no more connections open - shut down this
