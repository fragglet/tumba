	...
	if (lp_client_code_page() == KANJI_CODEPAGE) {
		...
		for (;;) {
			if (!*s || !*t)
				...
				diff = ((int) (unsigned char) *s) -
				       ((int) (unsigned char) *t);
				if (diff)
					return diff;
				diff = ((int) (unsigned char) *(s + 1)) -
				       ((int) (unsigned char) *(t + 1));
				if (diff)
					return diff;
				s += 2;
				t += 2;
...
#if !defined(KANJI_WIN95_COMPATIBILITY)
		/*
		 * For completeness we should put in equivalent code for code
		 * pages 949 (Korean hangul) and 950 (Big5 Traditional Chinese)
		 * here - but doubt anyone wants Samba to behave differently
		 * from Win95 and WinNT here. They both treat full width ascii
		 * characters as case senstive filenames (ie. they don't do the
		 * work we do here). JRA.
		 */

		if (lp_client_code_page() == KANJI_CODEPAGE) {
			/* Win95 treats full width ascii characters as case
			 * sensitive. */
			if (is_shift_jis(*s)) {
...
static BOOL matchname(char *remotehost, struct in_addr addr)
{
	...
	if (strcasecmp(remotehost, hp->h_name) &&
		...
		DEBUG(0, ("host name/name mismatch: %s != %s", remotehost,
		          hp->h_name));
		return False;
	}
	...
	DEBUG(0, ("host name/address mismatch: %s != %s", inet_ntoa(addr),
	          hp->h_name));
	return False;
}
...
variable [%s]\n",
			...
			if ((envval = getenv(envname)) == NULL) {
				DEBUG(0, ("standard_sub_basic: Environment "
				          "variable [%s] not set\n",
				          envname));
				p += 2;
				break;
	...
	for (; namelist->name != NULL; namelist++) {
		if (namelist->is_wild) {
			...
			if (unix_mask_match(last_component, namelist->name,
				...
				DEBUG(8, ("is_in_path: unix_mask_match "
				          "succeeded\n"));
				return True;
			}
		} else {
