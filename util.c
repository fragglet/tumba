...
void exchange_uids(void)
{
	if (geteuid() == 0 || getuid() == 0) {
		if (setreuid(geteuid(), getuid()) ||
			...
			Debug(0,
			      "Cannot exchange real/effective uid or gid!\n");
			close_sockets();
			exit(1);
		}
	}
...
int read_with_timeout(int fd, char *buf, int mincnt, int maxcnt, long time_out)
{
	...
	for (;;) {
		...
		if ((readret == 0) && nready) {
			...
			readret = read(
			    fd, buf + nread,
			    nready < maxcnt - nread ? nready : maxcnt - nread);
			if (readret == -1)
				return -1;
			nread += readret;
		}
...
again:
	...
	if (timeout > 0)
		ok = (read_with_timeout(fd, buffer + 4, len, len, timeout) ==
		      len);
	else
...
BOOL send_packet(char *buf, int len, struct in_addr *ip, int port, int type)
{
	...
	if (DEBUGLEVEL > 0)
		Debug(3,
		      "sending a packet of len %d to (%s) on port %d of type "
		      "%s\n",
...
BOOL name_query(char *inbuf, char *outbuf, char *name, struct in_addr *ip)
{
	...
	while (count--) {
		...
		show_nmb(outbuf);
		if (!send_packet(outbuf, nmb_len(outbuf), &bcast_ip, 137,
		                 SOCK_DGRAM)) {
			NeedSwap = saved_swap;
			return False;
		}
		...
		if (receive_nmb(inbuf, count == 1 ? 20 : 5)) {
			...
			if ((rec_name_trn_id = name_trn_id) && opcode == 0 &&
				...
				if (name_equal(qname, name)) {
					Debug(0,
					      "Someone (%s) gave us a positive "
					      "name query response\n",
					      inet_ntoa(lastip));
					memcpy(ip,
					       inbuf + 12 +
					           name_len(inbuf + 12) + 12,
					       4);
					NeedSwap = saved_swap;
					return (True);
				}
...
void stats_report(void)
{
	...
	while (time_stats[bin].thresh != -1) {
		if (time_stats[bin].count > 0)
			Debug(0,
			      "More than %3dk (%d)   %6d times    %g secs. %g "
			      "k/sec\n",
...
void *Realloc(void *p, int size)
{
	...
	if (!ret)
		Debug(
		    0,
		    "Memoory allocation error: failed to expand to %d bytes\n",
		    size);
...
BOOL register_name(name_struct *name, struct in_addr *destip, void (*fn)())
{
	...
	while (count--) {
		...
		show_nmb(outbuf);
		if (!send_packet(outbuf, nmb_len(outbuf), destip, 137,
		                 SOCK_DGRAM)) {
			free(inbuf);
			free(outbuf);
			NeedSwap = saved_swap;
		...
		if (receive_nmb(inbuf, 3)) {
			...
			if ((rec_name_trn_id == name_trn_id) &&
			    (opcode == 5 && nm_flags == 0x58 && rcode != 0 &&
			     rcode != 7)) {
				char qname[100];
				name_extract(inbuf, 12, qname);
	...
	{
		BOOL ret = send_packet(outbuf, nmb_len(outbuf), destip, 137,
		                       SOCK_DGRAM);
