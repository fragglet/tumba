...
static struct opcode_names {
	...
	int opcode;
} nmb_header_opcode_names[] = {{"Query", 0},
                               {"Registration", 5},
                               {"Release", 6},
                               {"WACK", 7},
                               {"Refresh", 8},
...
static void debug_nmb_res_rec(struct res_rec *res, char *hdr)
{
	...
	DEBUG(4,
	      ("    %s: nmb_name=%s rr_type=%d rr_class=%d ttl=%d\n", hdr,
	       namestr(&res->rr_name), res->rr_type, res->rr_class, res->ttl));
...
void debug_nmb_packet(struct packet_struct *p)
{
	...
	DEBUG(
	    4,
	    ("nmb packet from %s(%d) header: id=%d opcode=%s(%d) response=%s\n",
	     inet_ntoa(p->ip), p->port, nmb->header.name_trn_id,
	     lookup_opcode_name(nmb->header.opcode), nmb->header.opcode,
	     BOOLSTR(nmb->header.response)));
	DEBUG(4, ("    header: flags: bcast=%s rec_avail=%s rec_des=%s "
	          "trunc=%s auth=%s\n",
	...
	DEBUG(4, ("    header: rcode=%d qdcount=%d ancount=%d nscount=%d "
	          "arcount=%d\n",
	          nmb->header.rcode, nmb->header.qdcount, nmb->header.ancount,
	          nmb->header.nscount, nmb->header.arcount));
                             ...
                             BOOL *got_pointer, int *ret)
{
	...
	while ((ubuf[*offset] & 0xC0) == 0xC0) {
		...
		(*got_pointer) = True;
		(*offset) =
		    ((ubuf[*offset] & ~0xC0) << 8) | ubuf[(*offset) + 1];
		if (loop_count++ == 10 || (*offset) < 0 ||
		    (*offset) > (length - 2)) {
			return (False);
		}
	}
...
static int put_nmb_name(char *buf, int offset, struct nmb_name *name)
{
	...
	if (name->name[0] == '*') {
		...
		slprintf(buf1, sizeof(buf1) - 1, "%-15.15s%c", name->name,
		         name->name_type);
	}
...
char *namestr(struct nmb_name *n)
{
	...
	if (!n->scope[0])
		slprintf(p, sizeof(fstring) - 1, "%s<%02x>", n->name,
		         n->name_type);
	else
		slprintf(p, sizeof(fstring) - 1, "%s<%02x>.%s", n->name,
		         n->name_type, n->scope);
                                ...
                                struct res_rec **recs, int count)
{
	...
	*recs = (struct res_rec *) malloc(sizeof(**recs) * count);
	...
	for (i = 0; i < count; i++) {
		int l =
		    parse_nmb_name(inbuf, *offset, length, &(*recs)[i].rr_name);
		(*offset) += l;
		...
		memcpy((*recs)[i].rdata, inbuf + (*offset),
		       (*recs)[i].rdlength);
...
/*******************************************************************
  ...
  ******************************************************************/
static int put_compressed_name_ptr(unsigned char *buf, int offset,
                                   struct res_rec *rec, int ptr_offset)
{
	int ret = 0;
...
static BOOL parse_dgram(char *inbuf, int length, struct dgram_packet *dgram)
{
	...
	if (dgram->header.msg_type == 0x10 || dgram->header.msg_type == 0x11 ||
	    dgram->header.msg_type == 0x12) {
		offset +=
		    parse_nmb_name(inbuf, offset, length, &dgram->source_name);
		offset +=
		    parse_nmb_name(inbuf, offset, length, &dgram->dest_name);
	}
...
static BOOL parse_nmb(char *inbuf, int length, struct nmb_packet *nmb)
{
	...
	nmb->header.nm_flags.recursion_available =
	    (nm_flags & 8) ? True : False;
	nmb->header.nm_flags.recursion_desired =
	    (nm_flags & 0x10) ? True : False;
...
static struct packet_struct *copy_nmb_packet(struct packet_struct *packet)
{
	...
	if ((pkt_copy = (struct packet_struct *) malloc(sizeof(*packet))) ==
	    NULL) {
	...
	*pkt_copy = *packet;
	...
	if (nmb->answers) {
		if ((copy_nmb->answers = (struct res_rec *) malloc(
		         nmb->header.ancount * sizeof(struct res_rec))) == NULL)
			goto free_and_exit;
		memcpy((char *) copy_nmb->answers, (char *) nmb->answers,
		       nmb->header.ancount * sizeof(struct res_rec));
	}
	if (nmb->nsrecs) {
		if ((copy_nmb->nsrecs = (struct res_rec *) malloc(
		         nmb->header.nscount * sizeof(struct res_rec))) == NULL)
			goto free_and_exit;
		memcpy((char *) copy_nmb->nsrecs, (char *) nmb->nsrecs,
		       nmb->header.nscount * sizeof(struct res_rec));
	}
	if (nmb->additional) {
		if ((copy_nmb->additional = (struct res_rec *) malloc(
		         nmb->header.arcount * sizeof(struct res_rec))) == NULL)
			goto free_and_exit;
...
static int build_dgram(char *buf, struct packet_struct *p)
{
	...
	if (dgram->header.msg_type == 0x10 || dgram->header.msg_type == 0x11 ||
	    dgram->header.msg_type == 0x12) {
		offset +=
		    put_nmb_name((char *) ubuf, offset, &dgram->source_name);
		offset +=
		    put_nmb_name((char *) ubuf, offset, &dgram->dest_name);
	}
...
static int build_nmb(char *buf, struct packet_struct *p)
{
	...
	if ((nmb->header.response == False) &&
		...
		offset +=
		    put_compressed_name_ptr(ubuf, offset, nmb->additional, 12);
