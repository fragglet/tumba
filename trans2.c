	...
	*out_of_space = False;
	...
	while (!found) {
		...
		if (mask_match(fname, mask, case_sensitive, True)) {
			...
			mdate = sbuf.st_mtime;
			adate = sbuf.st_atime;
			cdate = get_create_time(
			    &sbuf, lp_fake_dir_create_times(SNUM(cnum)));
			if (mode & aDIR)
				size = 0;
                                    ...
                                    char **ppdata, int total_data)
{
	...
	case SMB_INFO_QUERY_EA_SIZE:
		...
		put_dos_date2(pdata, l1_fdateCreation,
		              get_create_time(
		                  &sbuf, lp_fake_dir_create_times(SNUM(cnum))));
	...
	case SMB_QUERY_FILE_BASIC_INFO:
		...
		DEBUG(5, ("SMB_QFBI - "));
		{
			time_t create_time = get_create_time(
			    &sbuf, lp_fake_dir_create_times(SNUM(cnum)));
...
int reply_trans2(char *inbuf, char *outbuf, int length, int bufsize)
{
  ...
  int32 timeout = IVALS(inbuf,smb_timeout);
	...
	if (global_oplock_break && (tran_call == TRANSACT2_OPEN)) {
		/*
		 * Queue this open message as we are the process of an oplock
		 * break.
		 */

		DEBUG(2, ("%s: reply_trans2: queueing message trans2open due "
