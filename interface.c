...
****************************************************************************/
static void get_broadcast(struct in_addr *if_ipaddr, struct in_addr *if_bcast,
                          struct in_addr *if_nmask)
{
    ...
    defined(_AIX41) || defined(__OpenBSD__)
	...
	/* sanity check on the netmask */
	{
		...
		while ((onbc + offbc) < 32) {
			if (nm & 0x80000000) {
				onbc++;
				if (offbc) /* already found an off bit, so mask
				              is wrong */
				{
					onbc = 34;
				}
	...
	{
		if_bcast->s_addr =
		    MKBCADDR(if_ipaddr->s_addr, if_nmask->s_addr);
	}
	...
	while (next_token(&ptr, token, NULL)) {
		...
		if (p) {
			if (strlen(p) > 2)
				...
				iface->nmask.s_addr =
				    htonl(((ALLONES >> atoi(p)) ^ ALLONES));
		} else {
			default_netmask(&iface->nmask, &iface->ip);
		}
		iface->bcast.s_addr =
		    MKBCADDR(iface->ip.s_addr, iface->nmask.s_addr);
		iface->next = NULL;
		...
		last_iface = iface;
		DEBUG(2,
		      ("Added %s ip=%s ", description, inet_ntoa(iface->ip)));
