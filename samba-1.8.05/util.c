...
/* info on the client */
struct from_host Client_info = {"UNKNOWN", "0.0.0.0", NULL};
...
void find_case_independent_name(char *name)
{
	...
	while (dptr) {
		if (strequal(dptr->d_name, search_name)) {
			DEBUG(3, ("find_case_independent_name - replacing %s "
			          "with %s\n",
...
BOOL file_exist(char *fname)
{
	...
	if ((Protocol >= PROTOCOL_LANMAN2) &&
	    casesignames) { /* We could use unix_convert_lanman2 here but
		               find_case... is a big optimization if this has
		               already been done (ie. we only need to seach for
		               the last component) */
...
********************************************************************/
void interpret_dos_date(uint32 date, int *year, int *month, int *day, int *hour,
...
time_t make_unix_date(void *date_ptr)
{
	...
	t.tm_yday = 1;
	t.tm_isdst = 0;
	/*  DEBUG(4,("year=%d month=%d day=%d hr=%d min=%d
...
time_t make_unix_date2(void *date_ptr)
{
	...
	DEBUG(4, ("year=%d month=%d day=%d hr=%d min=%d sec=%d\n", t.tm_year,
	          t.tm_mon, t.tm_mday, t.tm_hour, t.tm_sec));
	return TimeLocal(&t, GMT_TO_LOCAL);
}
...
static struct ino_list ino_head = {MAGIC, NULL, NULL,   (ino_t) 0, (dev_t) 0,
                                   0,     0,    1 << 8, NULL};
...
#define DISABLE_CACHE                                                          \
	{                                                                      \
		DEBUG(0, ("PLEASE REPORT THIS ERROR!\n"));                     \
		use_getwd_cache = False;                                       \
		return (Dumb_GetWd(s));                                        \
	}
	...
	/*  First a bit of housekeeping, we want to avoid 'total_count'
	   overflowing and going negative. This would cause havoc with the
	   weighting calculations used to keep the most commonly used
	   directories near the top of the list. As it is only used in a
	   relative manner, if it gets too large divide it, and the values it's
	   used with, by two. */
	...
	/*  Check whether I have this inode already. This is done with a simple
	    ...
	    Q: will this work with NFS/RFS/something-else-FS mounted
	   filesystems?

	    It's a linear search but the code tries to keep it sensible by
	   moving the most commonly used entries at the head of the list. */
	...
	for (ino_p = ino_head.next; ino_p; ino_p = ino_p->next) {
		...
		if (ino_p->back) {
			if (ino_p->back->next != ino_p) {
				DEBUG(0,
				      ("Hmm, back/next links inconsistant\n"));
				DISABLE_CACHE;
			}
			if (ino_p->back->back &&
			    ino_p->back->back->next->next != ino_p) {
				DEBUG(0, ("Hmm, back/back/next/next links "
				          "inconsistant\n"));
				DISABLE_CACHE;
			}
		}
		...
		/*  Calculate a weight for the entry and gradually sift less
		   used ones down the list by swapping. Note that the swapping
		   disconnects the backpointers so don't expect to run back up
		   the list. We keep a fair amount of granularity in the
		   'weight' value to avoid excessive swapping of elements.. it
		   is, after all, only a guide.. */

		ino_p->weight =
		    (ino_p->count << 8) / (total_count - ino_p->first);
		...
		if (ino_p->back && ino_p->weight > ino_p->back->weight) {
			...
			if (ino_tmp->back == NULL) {
				...
				DEBUG(
				    0,
				    ("Hmm, funny magic number looking back\n"));
				DISABLE_CACHE;
			}
#endif
		...
		/*  If we have found an entry with a matching inode and dev
		   number then find the inode number for the directory in the
		   cached string. If this agrees with that returned by the stat
		   for the current directory then all is o.k. (but make sure it
		   is a directory all the same...) */

		if (st.st_ino == ino_p->inode && st.st_dev == ino_p->dev) {
			if (stat(ino_p->text, &st2) == 0) {
				if (st.st_ino == st2.st_ino &&
					...
					DEBUG(
					    3,
					    ("Found cached GetWd %s [%d, %d]\n",
					     ino_p->text, i, ino_p->weight));
					strcpy(s, ino_p->text);
					return s;
				}

				/*  If the inode is different then something's
				   changed, scrub the entry and start from
				   scratch. */
		...
		/*  Set up a temporary backpointer, this will not be maintained
		   when elements are swapped so should not be relied on long
		   term. It helps however for immediate views back up the list.
		 */
	...
	/*  We'll prepend the new entry, prepending it means that it is at the
	   head of the list for the next request - working on the principle that
	   these lookups seldom come alone. */

	if (i > MAX_GETWDCACHE && ino_p) {
		free(ino_p->text);
		ino_p->back->next = NULL;
	} else if ((ino_p = (struct ino_list *) malloc(
	                sizeof(struct ino_list))) == NULL) {
		DEBUG(
		    0,
		    ("Oh dear, malloc failed extending list of directories\n"));
		strcpy(s, wd);
		return s;
	}
	ino_p->magic = MAGIC;
	...
	while (dptr) {
		if (strequal(dptr->d_name, name)) {
			...
			if (st.st_mode & flags) {
				...
				DEBUG(
				    3,
				    ("search_lanman2 - replacing %s with %s\n",
				     name, matchname));
				closedir(dirptr);
				return 0;
			}
...
void unix_convert_lanman2(char *s, char *home, BOOL case_is_sig)
{
	...
	while ((p = strchr(tnp, '/'))) {
		...
		/* Open the directory theoretically containing the
		   ...
		   version of it */
		if (search_lanman2(dir_to_search, name, matching_name,
		                   S_IFDIR)) {
			/* If we didn't find it then we may as well quit */
			strcpy(s, tmpname);

			DEBUG(5, ("search_lanman2 fail : Converted to lanman2 "
			          "name %s\n",
			          s));
	...
	if (*tnp) {
		...
		if (0 == search_lanman2(dir_to_search, tnp, matching_name,
		                        S_IFDIR | S_IFREG))
			memcpy(tnp, matching_name, strlen(matching_name));
	}
	strcpy(s, tmpname);
                ...
                BOOL doext)
{
	...
	if (strlen(mext) > 3)
		return False;
	if (strlen(nbeg) > strlen(mbeg))
		return False;
	if (strlen(next) > strlen(mext))
		return False;
...
int write_with_timeout(int fd, char *buf, int length, long time_out)
{
	...
	for (;;) {
		...
		do {
			selrtn = select(255, NULL, SELECT_CAST & fds, NULL,
			                &timeout);
		} while (selrtn < 0 && errno == EINTR);
...
BOOL send_keepalive(void)
{
	...
	while (nwritten < 4) {
		int ret =
		    write_socket(Client, (char *) &buf[nwritten], 4 - nwritten);
		if (ret <= 0)
			return False;
		nwritten += ret;
	}
...
BOOL read_data(int fd, char *buffer, int N)
{
	...
	if (maxtime > 0) {
		...
		while ((selrtn = select(255, SELECT_CAST & fds, NULL, NULL,
		                        &timeout)) == 0) {
...
int read_predict(int fd, int offset, char *buf, int num)
{
	...
	if (fd == rp_fd && offset >= rp_offset && possible > 0 &&
...
void do_read_prediction()
{
	...
	if (lseek(rp_fd, 0, SEEK_CUR) != rp_offset &&
	    lseek(rp_fd, rp_offset, SEEK_SET) != rp_offset)
		return;
...
int read_smb_length(int fd, char *inbuf, int timeout)
{
	...
	while (!ok) {
		if (timeout > 0)
			ok = (read_with_timeout(fd, buffer, 4, 4, timeout,
			                        False) == 4);
		else
	...
	while (!found && this_time - start_time <= maxtime) {
		...
		if (receive_nmb(inbuf, 1)) {
			...
			if ((rec_name_trn_id == name_trn_id) &&
				...
				char *p =
				    inbuf + 12 + name_len(inbuf + 12) + 10;
				int num_names = CVAL(p, 0);
				found = True;
				DEBUG(0, ("Got a positive node status response "
				          "from %s\n",
				          inet_ntoa(lastip)));
...
void replacestr(char *str1, char *str2, int start, int n)
{
	...
	else if (len > n) /* ie., opening up the string */
...
struct {
	...
	int tsize;
	int time;
} time_stats[] = {{0, 0, 0, 0},  {1, 0, 0, 0},   {5, 0, 0, 0},   {10, 0, 0, 0},
                  {50, 0, 0, 0}, {200, 0, 0, 0}, {500, 0, 0, 0}, {-1, 0, 0, 0}};
...
void stats_report(void)
{
	...
	while (time_stats[bin].thresh != -1) {
		if (time_stats[bin].count > 0)
			DEBUG(0, ("More than %3dk (%d)   %6d times    %g secs. "
			          "%g k/sec\n",
			          time_stats[bin].thresh,
			          time_stats[bin].tsize /
			              (1024 * time_stats[bin].count),
			          time_stats[bin].count,
			          time_stats[bin].time / 1.0e6,
			          (time_stats[bin].tsize / 1024.0) /
			              (time_stats[bin].time / 1.0e6)));
		bin++;
	}
}
...
BOOL sane_unix_date(time_t unixdate)
{
	...
	if (t.tm_year == today.tm_year && t.tm_mon > today.tm_mon)
		return False;
...
BOOL register_name(name_struct *name, struct in_addr *destip, void (*fn)())
{
	...
	if (name_trn_id == 0)
		name_trn_id = getpid() + (time(NULL) % 1000);
	...
	DEBUG(1, ("Registering name %s (%s) nb_flags=0x%x\n", name->name,
	          inet_ntoa(name->ip), name->nb_flags));
	...
	while (count--) {
		...
		if (receive_nmb(inbuf, 3)) {
			...
			if ((rec_name_trn_id == name_trn_id) &&
			    (opcode == 21 && nm_flags == 0x58 && rcode != 0 &&
			     rcode != 7)) {
				char qname[100];
				...
				if (name_equal(qname, name->name)) {
					DEBUG(
					    0,
					    ("Someone (%s) gave us a negative "
					     "name regregistration response!\n",
					     inet_ntoa(lastip)));
					free(inbuf);
					free(outbuf);
					NeedSwap = saved_swap;
					...
					DEBUG(0, ("%s gave negative name "
					          "regregistration for %s??\n",
					          inet_ntoa(lastip), qname));
				}
			}

			/* it's something else - process it anyway, unless we
			 are running as a daemon. This is necessary as we may
			 have been started by a name query of our name arriving
			 on port 137 (often happens) */
			if (fn) {
				show_nmb(inbuf);
				fn(inbuf, outbuf + nmb_len(outbuf));
			}
...
****************************************************************************/
void initgroups(name, id) char *name;
GID_TYPE id;
{
...
void *malloc_wrapped(int size, char *file, int line)
{
	...
	DEBUG(3, ("Malloc called from %s(%d) with size=%d gave ptr=0x%X\n",
	          file, line, size, (unsigned int) res));
	return res;
}
