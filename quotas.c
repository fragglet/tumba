...
BOOL disk_quotas(char *path, int *bsize, int *dfree, int *dsize)
{
	...
	*bsize = 1024;
	...
	/* Use softlimit to determine disk space, except when it has been
	 * exceeded */
	if ((D.dqb_bsoftlimit && D.dqb_curblocks >= D.dqb_bsoftlimit) ||
...
BOOL disk_quotas(char *path, int *bsize, int *dfree, int *dsize)
{
	...
	/* find the block device file */
	if ((stat(path, &S) < 0) ||
	    (devnm(S_IFBLK, S.st_dev, dev_disk, 256, 0) < 0))
		return (False);
#endif
...
#ifdef USE_SETRES
	{
		...
		/* for HPUX, real uid must be same as euid to execute quotactl
		 * for euid */
		user_id = getuid();
...
#if defined(__FreeBSD__) || defined(__OpenBSD__)
	{
		...
		setuid(0);
		seteuid(0);
		r = quotactl(path, QCMD(Q_GETQUOTA, USRQUOTA), euser_id,
		             (char *) &D);

		/* As FreeBSD has group quotas, if getting the user
		   quota fails, try getting the group instead. */
		if (r)
			r = quotactl(path, QCMD(Q_GETQUOTA, GRPQUOTA), egrp_id,
			             (char *) &D);
		setuid(user_id);
		seteuid(euser_id);
	}
