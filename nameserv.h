 ...
 * REGISTER_NAME, DNS_NAME, DNSFAIL_NAME should only be in the
 * wins_server_subnet. WINS_PROXY_NAME should only be on the broadcast subnets.
...
struct name_record {
	...
	time_t death_time; /* The time the record must be removed (do not remove
	                      if 0). */
...
struct work_record {
	...
	/* Stage of development from non-local-master up to local-master
	 * browser. */
...
typedef void (*response_function)(struct subnet_record *,
                                  struct response_record *,
                                  struct packet_struct *);
...
typedef void (*success_function)(struct subnet_record *,
                                 struct userdata_struct *, ...);
