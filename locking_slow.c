                                ...
                                share_mode_entry **old_shares)
{
	...
	*/
	...
	if (read_share_file(cnum, fd, fname, &buf, &new_file) != 0) {
		DEBUG(0,
		      ("ERROR: get_share_modes: Failed to read share file %s\n",
		       fname));
		return 0;
	}
	...
	if (num_entries < 0) {
		DEBUG(
		    0,
		    ("PANIC ERROR:get_share_mode: num_share_mode_entries < 0 (%d) \
for share file %s\n",
		     num_entries, fname));
		return 0;
	}
	...
	if (num_entries) {
		*old_shares = share_array = (share_mode_entry *) malloc(
		...
		DEBUG(0, ("get_share_modes: share file %s has no share mode "
		          "entries - deleting.\n",
		          fname));
		if (buf)
			free(buf);
	...
	for (i = 0; i < num_entries; i++) {
		...
		if (!process_exists(pid)) {
			DEBUG(
			    0,
			    ("get_share_modes: process %d no longer exists and \
it left a share mode entry with mode 0x%X in share file %s\n",
			     pid, IVAL(p, SME_SHAREMODE_OFFSET), fname));
			continue;
		}
		share_array[num_entries_copied].time.tv_sec =
		    IVAL(p, SME_SEC_OFFSET);
		share_array[num_entries_copied].time.tv_usec =
		    IVAL(p, SME_USEC_OFFSET);
		share_array[num_entries_copied].share_mode =
		    IVAL(p, SME_SHAREMODE_OFFSET);
		share_array[num_entries_copied].pid = pid;
		share_array[num_entries_copied].op_port =
		    SVAL(p, SME_PORT_OFFSET);
		share_array[num_entries_copied].op_type =
		    SVAL(p, SME_OPLOCK_TYPE_OFFSET);
	...
	if (num_entries_copied == 0) {
		...
		DEBUG(0, ("get_share_modes: share file %s had no valid entries "
		          "- deleting it !\n",
		          fname));
		if (*old_shares)
	...
	if (num_entries_copied != num_entries) {
		if (lseek(fd, 0, SEEK_SET) != 0) {
			DEBUG(
			    0,
			    ("ERROR: get_share_modes: lseek failed to reset to \
position 0 for share mode file %s (%s)\n",
		...
		for (i = 0; i < num_entries_copied; i++) {
			...
			SIVAL(p, SME_SHAREMODE_OFFSET,
			      share_array[i].share_mode);
			...
			SSVAL(p, SME_OPLOCK_TYPE_OFFSET,
			      share_array[i].op_type);
		}

		newsize =
		    (base - buf) + (SMF_ENTRY_LENGTH * num_entries_copied);
		if (write(fd, buf, newsize) != newsize) {
			DEBUG(
			    0,
			    ("ERROR: get_share_modes: failed to re-write share \
mode file %s (%s)\n",
			     fname, strerror(errno)));
			if (*old_shares)
				free((char *) *old_shares);
		...
		if (ftruncate(fd, newsize) != 0) {
			...
			DEBUG(
			    0,
			    ("ERROR: get_share_modes: failed to ftruncate share \
...
static void slow_del_share_mode(int token, int fnum)
{
	...
	if (read_share_file(fs_p->cnum, fd, fname, &buf, &new_file) != 0) {
		DEBUG(0,
		      ("ERROR: del_share_mode: Failed to read share file %s\n",
		       fname));
		return;
	}

	if (new_file == True) {
		DEBUG(0, ("ERROR:del_share_mode: share file %s is new (size "
		          "zero), deleting it.\n",
		          fname));
	...
	if (num_entries < 0) {
		DEBUG(
		    0,
		    ("PANIC ERROR:del_share_mode: num_share_mode_entries < 0 (%d) \
for share file %s\n",
		     num_entries, fname));
		return;
	}
	...
	if (num_entries == 0) {
		/* No entries - just delete the file. */
		DEBUG(0, ("del_share_mode: share file %s has no share mode "
		          "entries - deleting.\n",
		          fname));
		if (buf)
			free(buf);
	...
	*/
	...
	for (i = 0; i < num_entries; i++) {
		...
		DEBUG(5, ("del_share_mode: deleting entry number %d (of %d) "
		          "from the share file %s\n",
		          i, num_entries, fname));

		/* Remove this entry. */
		if (i != num_entries - 1)
			memcpy(p, p + SMF_ENTRY_LENGTH,
			       (num_entries - i - 1) * SMF_ENTRY_LENGTH);
	...
	if (num_entries == 0) {
		...
		DEBUG(5, ("del_share_mode: removed last entry in share file - "
		          "deleting share file %s\n",
		          fname));
		if (buf)
			free(buf);
		delete_share_file(fs_p->cnum, fname);
...
********************************************************************/
static BOOL slow_set_share_mode(int token, int fnum, uint16 port,
                                uint16 op_type)
{
	files_struct *fs_p = &Files[fnum];
	pstring fname;
	...
	if (fstat(fd, &sb) != 0) {
		DEBUG(0, ("ERROR: set_share_mode: Failed to do stat on share "
		          "file %s\n",
		          fname));
		return False;
	}
	...
	if (sb.st_size != 0) {
		...
		if ((buf = (char *) malloc(sb.st_size + SMF_ENTRY_LENGTH)) ==
		    NULL) {
			DEBUG(
			    0,
			    ("set_share_mode: malloc for file size %d fail !\n",
		...
		if (lseek(fd, 0, SEEK_SET) != 0) {
			DEBUG(
			    0,
			    ("ERROR: set_share_mode: Failed to reset position \
to 0 for share file %s (%s)\n",
		...
		if (read(fd, buf, sb.st_size) != sb.st_size) {
			DEBUG(0, ("ERROR: set_share_mode: Failed to read share "
			          "file %s (%s)\n",
			          fname, strerror(errno)));
			if (buf)
				free(buf);
		...
		if (IVAL(buf, SMF_VERSION_OFFSET) != LOCKING_VERSION) {
			DEBUG(
			    0,
			    ("ERROR: set_share_mode: share file %s has incorrect \
locking version (was %d, should be %d).\n",
		...
		size -= (SMF_HEADER_LENGTH +
		         SVAL(buf,
		              SMF_FILENAME_LEN_OFFSET)); /* Remove the header */

		/* The remaining size must be a multiple of SMF_ENTRY_LENGTH -
		 * error if not. */
		if ((size % SMF_ENTRY_LENGTH) != 0) {
			DEBUG(
			    0,
			    ("ERROR: set_share_mode: share file %s is an incorrect length - \
deleting it.\n",
			     fname));
			if (buf)
				free(buf);
		...
		if ((buf = (char *) malloc(SMF_HEADER_LENGTH +
		                           strlen(fs_p->name) + 1 +
		                           SMF_ENTRY_LENGTH)) == NULL) {
			DEBUG(0, ("ERROR: set_share_mode: malloc failed for "
			          "single entry.\n"));
			return False;
		}
	...
	if (lseek(fd, 0, SEEK_SET) != 0) {
		DEBUG(0,
		      ("ERROR: set_share_mode: (1) Failed to reset position to \
0 for share file %s (%s)\n",
	...
	if (write(fd, buf, header_size + (num_entries * SMF_ENTRY_LENGTH)) !=
		...
		DEBUG(2,
		      ("ERROR: set_share_mode: Failed to write share file %s - \
deleting it (%s).\n",
		       fname, strerror(errno)));
		delete_share_file(fs_p->cnum, fname);
		if (buf)
	...
	if (ftruncate(fd, header_size + (SMF_ENTRY_LENGTH * num_entries)) !=
	    0) {
