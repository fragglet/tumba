 ...
 */
static int pc1[56] = {57, 49, 41, 33, 25, 17, 9,  1,  58, 50, 42, 34, 26, 18,
 ...
 */
static int rots[16] = {1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};

/*
 * Permutation giving the key
 * of the i' DES round
 */
static int pc2[48] = {14, 17, 11, 24, 1,  5,  3,  28, 15, 6,  21, 10,
 ...
 */
static int esel[48] = {32, 1,  2,  3,  4,  5,  4,  5,  6,  7,  8,  9,
 ...
 */
static int perm32[32] = {16, 7, 20, 21, 29, 12, 28, 17, 1,  15, 23,
 ...
 * permutation matrix
 */
static int final_perm[64] = {40, 8,  48, 16, 56, 24, 64, 32, 39, 7,  47, 15, 55,
...
static unsigned char bytemask[8] = {0x80, 0x40, 0x20, 0x10,
                                    0x08, 0x04, 0x02, 0x01};
...
static void ufc_init_des()
{
	...
	for (bit = 0; bit < 56; bit++) {
		...
		for (j = 0; j < 128; j++) {
			if (j & mask1)
				do_pc1[comes_from_bit / 8][bit / 28][j] |=
				    mask2;
		}
	}
	...
	for (sg = 0; sg < 4; sg++) {
		...
		for (j1 = 0; j1 < 64; j1++) {
			...
			for (j2 = 0; j2 < 64; j2++) {
				...
				inx = ((j1 << 6) | j2) << 1;
				sb[sg][inx] =
				    eperm32tab[0][(to_permute >> 24) & 0xff][0];
				sb[sg][inx + 1] =
				    eperm32tab[0][(to_permute >> 24) & 0xff][1];
				sb[sg][inx] |=
				    eperm32tab[1][(to_permute >> 16) & 0xff][0];
				sb[sg][inx + 1] |=
				    eperm32tab[1][(to_permute >> 16) & 0xff][1];
				sb[sg][inx] |=
				    eperm32tab[2][(to_permute >> 8) & 0xff][0];
				sb[sg][inx + 1] |=
				    eperm32tab[2][(to_permute >> 8) & 0xff][1];
				sb[sg][inx] |=
				    eperm32tab[3][(to_permute) & 0xff][0];
				sb[sg][inx + 1] |=
				    eperm32tab[3][(to_permute) & 0xff][1];
#endif
#ifdef _UFC_64_
				inx = ((j1 << 6) | j2);
				sb[sg][inx] =
				    ((long64) eperm32tab[0][(to_permute >> 24) &
				                            0xff][0]
				     << 32) |
				    (long64) eperm32tab[0][(to_permute >> 24) &
				                           0xff][1];
				sb[sg][inx] |=
				    ...
				    (long64) eperm32tab[1][(to_permute >> 16) &
				                           0xff][1];
				sb[sg][inx] |=
				    ((long64) eperm32tab[2][(to_permute >> 8) &
				                            0xff][0]
				     << 32) |
				    (long64) eperm32tab[2][(to_permute >> 8) &
				                           0xff][1];
	...
	for (bit = 0; bit < 64; bit++) {
		...
		for (word_value = 64; word_value--;) {
			if (word_value & mask1)
				efp[comes_from_word][word_value][o_long] |=
				    mask2;
		}
	}
	initialized++;
...
#ifdef _UFC_32_
static void shuffle_sb(k, saltbits) long32 *k;
ufc_long saltbits;
{
	ufc_long j;
	long32 x;
...
#ifdef _UFC_64_
static void shuffle_sb(k, saltbits) long64 *k;
ufc_long saltbits;
{
	ufc_long j;
	long64 x;
...
static void ufc_mk_keytab(key) char *key;
{
	ufc_long v1, v2, *k1;
	int i;
#ifdef _UFC_32_
...
ufc_long l1, l2, r1, r2;
{
	...
	x = (l1 ^ l2) & current_saltbits;
	l1 ^= x;
	l2 ^= x;
	x = (r1 ^ r2) & current_saltbits;
	r1 ^= x;
	r2 ^= x;
...
char *ufc_crypt(char *key, char *salt)
{
	 ...
	 */
	s = _ufc_doit((ufc_long) 0, (ufc_long) 0, (ufc_long) 0, (ufc_long) 0,
	              (ufc_long) 25);
...
ufc_long l1, l2, r1, r2, itr;
{
	...
	while (itr--) {
		...
		for (i = 8; i--;) {
			s = *k++ ^ r1;
			l1 ^= SBA(_ufc_sb1, s & 0xffff);
			l2 ^= SBA(_ufc_sb1, (s & 0xffff) + 4);
			l1 ^= SBA(_ufc_sb0, s >>= 16);
			l2 ^= SBA(_ufc_sb0, (s) + 4);
			s = *k++ ^ r2;
			l1 ^= SBA(_ufc_sb3, s & 0xffff);
			l2 ^= SBA(_ufc_sb3, (s & 0xffff) + 4);
			l1 ^= SBA(_ufc_sb2, s >>= 16);
			l2 ^= SBA(_ufc_sb2, (s) + 4);

			s = *k++ ^ l1;
			r1 ^= SBA(_ufc_sb1, s & 0xffff);
			r2 ^= SBA(_ufc_sb1, (s & 0xffff) + 4);
			r1 ^= SBA(_ufc_sb0, s >>= 16);
			r2 ^= SBA(_ufc_sb0, (s) + 4);
			s = *k++ ^ l2;
			r1 ^= SBA(_ufc_sb3, s & 0xffff);
			r2 ^= SBA(_ufc_sb3, (s & 0xffff) + 4);
			r1 ^= SBA(_ufc_sb2, s >>= 16);
			r2 ^= SBA(_ufc_sb2, (s) + 4);
		}
		s = l1;
		l1 = r1;
