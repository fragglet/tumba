...
static BOOL enumerate_sections(FILE *infile, PM_SECFUNC sfunc,
                               PM_PARMFUNC pfunc);
	...
	while (True) {
		...
		if ((lFileOffset = ftell(fileIn)) >= 0L) {
			...
			if (fgets(szBuf, PM_MAXLINE, fileIn) == NULL) {
				...
				if (bRetval)
					Debug(3, "End of file encountered "
				...
				/* if first non-white is a '[', stop (new
				 * section) */
				if ((cTemp = firstnonwhite(szBuf)) == '[') {
					/* restore position to start of new
					 * section */
					if (fseek(fileIn, lFileOffset,
					          SEEK_SET) < 0L) {
						Debug(0,
						      "Seek error on "
						      "configuration file!\n");
					...
					/* if it's a semicolon or line is blank,
					 * ignore the line */
					if (cTemp == ';' || cTemp == '\0') {
						continue;
					} else
						/* if no equals sign and line
...
**************************************************************************/
static BOOL enumerate_sections(FILE *fileIn, PM_SECFUNC sfunc,
                               PM_PARMFUNC pfunc)
{
	char szBuf[PM_MAXLINE + 1];
	BOOL bRetval;
	...
	while (True) {
		if (fgets(szBuf, PM_MAXLINE, fileIn) == NULL) {
			...
			bRetval = !ferror(fileIn);
			if (bRetval)
				Debug(3, "End of file encountered (enumerating "
				         "sections)!\n");
			else
				Debug(0, "Read error on configuration file "
			...
			if (szBuf[0] == '[') {
				...
				if (strlen(szBuf) > 1)
					if (szBuf[strlen(szBuf) - 1] == ']') {
						 ...
						 * whitespace from name */
						trimleft(szBuf);
						trimright(szBuf);
						/* reduce all internal
						 * whitespace to one space */
						collapse_spaces(szBuf);
						/* process it - stop if the
						 * processing fails */
						if (sfunc != NULL)
							if (!sfunc(szBuf))
								break;
						if (!enumerate_parameters(
						        fileIn, pfunc))
							break;
					}
			}
	...
	if (bRetval)
		if (!bSectionFound)
			Debug(0,
			      "Configuration file contains no sections (!?)\n");
	return (bRetval);
}
...
BOOL pm_process(char *pszFileName, PM_SECFUNC sfunc, PM_PARMFUNC pfunc)
{
	...
	else if ((fileIn = fopen(pszParmFile, "rt")) == NULL)
