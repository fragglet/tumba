...
err_code_struct server_msgs[] = {
    ...
    {"ERRsrverror", 65,
     "The server encountered an internal error, e.g., system file "
     "unavailable."},
    {"ERRfilespecs", 67,
     "The file handle (FID) and pathname parameters contained an invalid  "
...
} err_classes[] = {{0, "SUCCESS", NULL},          {0x01, "ERRDOS", dos_msgs},
                   {0x02, "ERRSRV", server_msgs}, {0x03, "ERRHRD", hard_msgs},
                   {0x04, "ERRXOS", NULL},        {0xE1, "ERRRMX1", NULL},
                   {0xE2, "ERRRMX2", NULL},       {0xE3, "ERRRMX3", NULL},
                   {0xFF, "ERRCMD", NULL},        {-1, NULL, NULL}};
...
char *smb_errstr(char *inbuf)
{
	...
	for (i = 0; err_classes[i].class; i++)
		if (err_classes[i].code == class) {
			...
			slprintf(ret, sizeof(pstring) - 1, "%s - %d",
			         err_classes[i].class, num);
			return ret;
		}

	slprintf(ret, sizeof(pstring) - 1, "Error: Unknown error (%d,%d)",
	         class, num);
	return (ret);
}
