...
/****************************************************************************
  ...
  ****************************************************************************/
static BOOL cli_send_trans(struct cli_state *cli, int trans, char *name,
                           int fid, int flags, char *data, char *param,
                           ...
                           int mdata, int mparam, int msetup)
{
	...
	this_lparam =
	    MIN(lparam, cli->max_xmit - (500 + lsetup * 2)); /* hack */
	this_ldata =
	    MIN(ldata, cli->max_xmit - (500 + lsetup * 2 + this_lparam));
	...
	outparam =
	    smb_buf(cli->outbuf) + (trans == SMBtrans ? strlen(name) + 1 : 3);
	...
	SSVAL(cli->outbuf, smb_psoff,
	      smb_offset(outparam, cli->outbuf));  /* psoff */
	SSVAL(cli->outbuf, smb_dscnt, this_ldata); /* dscnt */
	SSVAL(cli->outbuf, smb_dsoff,
	      smb_offset(outdata, cli->outbuf)); /* dsoff */
	...
	set_message(cli->outbuf, 14 + lsetup, /* wcnt, bcc */
	            PTR_DIFF(outdata + this_ldata, smb_buf(cli->outbuf)),
	            False);
	...
	if (this_ldata < ldata || this_lparam < lparam) {
		...
		while (tot_data < ldata || tot_param < lparam) {
			this_lparam = MIN(lparam - tot_param,
			                  cli->max_xmit - 500); /* hack */
			this_ldata = MIN(ldata - tot_data,
			                 cli->max_xmit - (500 + this_lparam));

			set_message(cli->outbuf, trans == SMBtrans ? 8 : 9, 0,
			            True);
			CVAL(cli->outbuf, smb_com) =
			    trans == SMBtrans ? SMBtranss : SMBtranss2;
			...
			if (this_ldata) /* data[] */
				memcpy(outdata, data, this_ldata);
			set_message(cli->outbuf,
			            trans == SMBtrans ? 8 : 9, /* wcnt, bcc */
			            PTR_DIFF(outdata + this_ldata,
			                     smb_buf(cli->outbuf)),
			            False);
...
/****************************************************************************
  ...
  ****************************************************************************/
static BOOL cli_receive_trans(struct cli_state *cli, int trans, int *data_len,
	...
	while (1) {
		...
		if (this_data)
			memcpy(*data + SVAL(cli->inbuf, smb_drdisp),
			       smb_base(cli->inbuf) +
			           SVAL(cli->inbuf, smb_droff),
			       this_data);
		if (this_param)
			memcpy(*param + SVAL(cli->inbuf, smb_prdisp),
			       smb_base(cli->inbuf) +
			           SVAL(cli->inbuf, smb_proff),
			       this_param);
		*data_len += this_data;
...
call a remote api
****************************************************************************/
static BOOL cli_api(struct cli_state *cli, int prcnt, int drcnt, int mprcnt,
                    int mdrcnt, int *rprcnt, int *rdrcnt, char *param,
                    char *data, char **rparam, char **rdata)
{
	cli_send_trans(cli, SMBtrans, PIPE_LANMAN, 0, 0, data, param, NULL,
	               drcnt, prcnt, 0, mdrcnt, mprcnt, 0);

	return (
	    cli_receive_trans(cli, SMBtrans, rdrcnt, rprcnt, rdata, rparam));
}
...
BOOL cli_NetWkstaUserLogon(struct cli_state *cli, char *user, char *workstation)
{
	...
	if (cli_api(cli, PTR_DIFF(p, param), 0, 1024, BUFFER_SIZE, &rprcnt,
	            &rdrcnt, param, NULL, &rparam, &rdata)) {
                       ...
                       void (*fn)(char *, uint32, char *))
{
	...
	if (cli_api(cli, PTR_DIFF(p, param), /* param count */
	            0,                       /*data count */
	            8,                       /* mprcount */
	            BUFFER_SIZE,             /* mdrcount */
	            &rprcnt, &rdrcnt, param, NULL, &rparam, &rdata)) {
		...
		if (res == 0) {
			...
			for (i = 0; i < count; i++, p += 26) {
				char *sname = p;
				int comment_offset =
				    (IVAL(p, 22) & 0xFFFF) - converter;
				char *cmnt = comment_offset
				               ? (rdata + comment_offset)
				               : "";
				if (comment_offset < 0 ||
				    comment_offset > rdrcnt)
					continue;
...
static struct {
	int prot;
	char *name;
} prots[] = {{PROTOCOL_CORE, "PC NETWORK PROGRAM 1.0"},
                       ...
                       char *workgroup)
{
	...
	if (((passlen == 0) || (passlen == 1)) && (pass[0] == '\0')) {
		...
		if ((cli->sec_mode & 2) && passlen != 24) {
			passlen = 24;
			SMBencrypt((uchar *) pass, (uchar *) cli->cryptkey,
			           (uchar *) pword);
		} else {
	...
	if (cli->protocol < PROTOCOL_NT1) {
		...
		pstrcpy(p, "Samba");
		p = skip_string(p, 1);
		set_message(cli->outbuf, 13, PTR_DIFF(p, smb_buf(cli->outbuf)),
		            False);
	}
                    ...
                    int passlen)
{
	...
	slprintf(fullshare, sizeof(fullshare) - 1, "\\\\%s\\%s", cli->desthost,
	         share);
                   ...
                   time_t *a_time, time_t *m_time, uint32 *size)
{
	...
	if (!cli_send_trans(cli, SMBtrans2, NULL, -1, 0, NULL, param, &setup,
	                    data_len, param_len, 1, cli->max_xmit, 10, 0)) {
		return False;
	}

	if (!cli_receive_trans(cli, SMBtrans2, &data_len, &param_len, &rdata,
	                       &rparam)) {
		return False;
	}
                   ...
                   time_t *a_time, time_t *m_time, uint32 *size)
{
	...
	if (!cli_send_trans(cli, SMBtrans2, NULL, -1, 0, NULL, param, &setup,
	                    data_len, param_len, 1, cli->max_xmit, 2, 0)) {
		return False;
	}
                             ...
                             char *new_password, char *old_password)
{
	...
	if (strlen(user) >= sizeof(fstring) - 1) {
		DEBUG(0,
		      ("cli_oem_change_password: user name %s is too long.\n",
		       user));
		return False;
	}

	if (new_pw_len > 512) {
		DEBUG(0, ("cli_oem_change_password: new password for user %s "
		          "is too long.\n",
		          user));
		return False;
	}
	...
	if (cli_send_trans(cli, SMBtrans, PIPE_LANMAN, 0, 0, data, param, NULL,
	                   data_len, param_len, 0, 0, 2, 0) == False) {
		DEBUG(0, ("cli_oem_change_password: Failed to send password "
		          "change for user %s\n",
		          user));
		return False;
	}

	if (cli_receive_trans(cli, SMBtrans, &rdrcnt, &rprcnt, &rdata,
	                      &rparam)) {
		if (rparam)
			cli->error = SVAL(rparam, 0);
	}
...
BOOL cli_negprot(struct cli_state *cli)
{
	...
	if (cli->protocol >= PROTOCOL_NT1) {
		...
		cli->servertime =
		    interpret_long_date(cli->inbuf + smb_vwv11 + 1);
		memcpy(cli->cryptkey, smb_buf(cli->inbuf), 8);
		if (IVAL(cli->inbuf, smb_vwv9 + 1) & 1)
			cli->readbraw_supported = cli->writebraw_supported =
			    True;
		...
		cli->readbraw_supported =
		    ((SVAL(cli->inbuf, smb_vwv5) & 0x1) != 0);
		cli->writebraw_supported =
		    ((SVAL(cli->inbuf, smb_vwv5) & 0x2) != 0);
		memcpy(cli->cryptkey, smb_buf(cli->inbuf), 8);
	} else {
