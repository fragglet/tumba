...
static int add_a_service(service *pservice)
{
	...
	if (iNumServices == 0)
		...
		service *ptmpservice = (service *) realloc(
		    pServices, sizeof(service) * (iNumServices + 1));
		if (!ptmpservice) {
...
static int getservicebyname(char *pszServiceName, service *pserviceDest)
{
	...
	for (iService = iServiceIndex - 1; iService >= 0; iService--)
		if (strwicmp(pServices[iService].szService, pszServiceName) ==
		    0) {
			if (pserviceDest != NULL)
				copy_service(pserviceDest, &pServices[iService],
				             NULL);
			break;
		}
                         ...
                         copymap *pcopymapDest)
{
	...
	if (bcopyall || pcopymapDest->bUS_dont_descend)
		string_set(&pserviceDest->szDontdescend,
		           pserviceSource->szDontdescend);
	...
	if (bcopyall || pcopymapDest->bUS_map_hidden)
		pserviceDest->bMap_hidden = pserviceSource->bMap_hidden;

	if (bcopyall || pcopymapDest->bUS_create_mode)
		pserviceDest->iCreate_mode = pserviceSource->iCreate_mode;
...
static BOOL do_service_parameter(parmmap *pparmmap, char *pszParmValue)
{
	...
	case E_COPY:
		bRetval = False;
		string_set(&COPYMAPS(iServiceIndex).szSourceService,
		           pszParmValue);
		COPYMAPS(iServiceIndex).bCopyEncountered = True;
		Debug(2, "Copying service from service %s\n", pszParmValue);
		if ((iTemp = getservicebyname(pszParmValue, &serviceTemp)) >=
		    0) {
			if (iTemp == iServiceIndex)
				Debug(0,
				      "Can't copy service %s- unable to copy "
				      "self!\n",
				      pszParmValue);
			else if (COPYMAPS(iTemp).bCopyEncountered)
				Debug(0,
				      "Can't copy service %s - source uses "
				      "copy too!\n",
				      pszParmValue);
			else {
				...
				COPYMAPS(iServiceIndex).bCopyEncountered =
				    False;
				bRetval = True;
			}
		} else
	...
	case E_DONTDESCEND:
		string_set(&pServices[iServiceIndex].szDontdescend,
		           pszParmValue);
	...
	case E_CREATEMODE:
		bRetval = (sscanf(pszParmValue, "%o",
		                  &pServices[iServiceIndex].iCreate_mode) == 1);
...
static BOOL do_parameter(char *pszParmName, char *pszParmValue)
{
	...
	case E_SERVICE:
		if (bInGlobalSection)
			Debug(0,
			      "Service parameter %s found in global section!\n",
			      pszParmName);
		else
			bRetval =
			    do_service_parameter(&parmmapTemp, pszParmValue);
		break;
	...
	case E_GLOBAL:
		if (!bInGlobalSection)
			...
			bRetval =
			    do_global_parameter(&parmmapTemp, pszParmValue);
		break;
	...
	default:
		Debug(
		    0,
		    "The following two messages indicate an internal error:\n");
...
static BOOL do_section(char *pszSectionName)
{
	...
	if (bInGlobalSection) {
		...
		if (bDoneGlobalSection)
			Debug(0, "Multiple global sections found in "
			         "configuration file!\n");
		else {
	...
	if (iServiceIndex >= iNumServices) {
		Debug(0,
		      "Maximum service count (%d) exceeded at service \"%s\"\n",
		...
		/* if all is still well, move to the next record in the services
		 * array */
		if (bRetval) {
			/* We put this here to avoid an odd message order if
			 * messages are */
			...
			string_init(&pServices[iServiceIndex].szService,
			            pszSectionName);
		}
	}
	return (bRetval);
...
{
	return (LP_SNUM_OK(iService) ? pServices[iService].iCreate_mode
	                             : Globals.iCreate_mode);
}
