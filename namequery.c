                           ...
                           void (*fn)(struct packet_struct *))
{
	...
	if (!name_trn_id)
		name_trn_id = (time(NULL) % (unsigned) 0x7FFF) +
		              (getpid() % (unsigned) 100);
	...
	while (1) {
		...
		if ((p2 = receive_packet(fd, NMB_PACKET, 90))) {
			...
			if (nmb->header.name_trn_id !=
			        nmb2->header.name_trn_id ||
			...
			if (nmb2->header.opcode != 0 ||
			    nmb2->header.nm_flags.bcast || nmb2->header.rcode ||
			    !nmb2->header.ancount) {
				/* XXXX what do we do with this? could be a
				   redirect, but we'll discard it for the moment
				 */
				free_packet(p2);
				continue;
			}

			ip_list = (struct in_addr *) Realloc(
			    ip_list,
			    sizeof(ip_list[0]) *
			        ((*count) + nmb2->answers->rdlength / 6));
			if (ip_list) {
				DEBUG(fn ? 3 : 2, ("Got a positive name query "
				                   "response from %s ( ",
				                   inet_ntoa(p2->ip)));
				for (i = 0; i < nmb2->answers->rdlength / 6;
				     i++) {
					putip((char *) &ip_list[(*count)],
					      &nmb2->answers->rdata[2 + i * 6]);
					DEBUG(fn ? 3 : 2,
					      ("%s ",
					       inet_ntoa(ip_list[(*count)])));
					(*count)++;
				}
				DEBUG(fn ? 3 : 2, (")\n"));
			}
...
FILE *startlmhosts(char *fname)
{
	...
	if (!fp) {
		DEBUG(
		    2,
		    ("startlmhosts: Can't open lmhosts file %s. Error was %s\n",
...
BOOL getlmhostsent(FILE *fp, char *name, int *name_type, struct in_addr *ipaddr)
{
	...
	while (!feof(fp) && !ferror(fp)) {
		...
		*name_type = -1;
		...
		if (count >= 4) {
			DEBUG(0, ("getlmhostsent: too many columns in lmhosts "
			          "file (obsolete syntax)\n"));
			continue;
		}
		...
		if (strchr(flags, 'G') || strchr(flags, 'S')) {
			DEBUG(0, ("getlmhostsent: group flag in lmhosts "
			          "ignored (obsolete)\n"));
			continue;
		}
		...
		*ipaddr = *interpret_addr2(ip);
		...
		/* Extra feature. If the name ends in '#XX', where XX is a hex
		   number, then only add that name type. */
		if ((ptr = strchr(name, '#')) != NULL) {
			...
			*name_type = (int) strtol(ptr, &endptr, 0);
			...
			if (!*ptr || (endptr == ptr)) {
				DEBUG(0, ("getlmhostsent: invalid name %s "
				          "containing '#'.\n",
				          name));
				continue;
			}
...
BOOL resolve_name(char *name, struct in_addr *return_ip)
{
	...
	while (next_token(&ptr, tok, LIST_SEP)) {
		if (strequal(tok, "host") || strequal(tok, "hosts")) {
			...
			DEBUG(3, ("resolve_name: Attempting host lookup for "
			          "name %s\n",
			          name));

			if (((hp = Get_Hostbyname(name)) != NULL) &&
			    (hp->h_addr != NULL)) {
			...
			DEBUG(3, ("resolve_name: Attempting lmhosts lookup for "
			          "name %s\n",
			          name));

			fp = startlmhosts(LMHOSTSFILE);
			if (fp) {
				while (getlmhostsent(fp, lmhost_name,
				                     &name_type, return_ip)) {
			...
			/*
			 * "bcast" means do a broadcast lookup on all the local
			 * interfaces.
			 */

			DEBUG(3, ("resolve_name: Attempting broadcast lookup "
			          "for name %s<0x20>\n",
			          name));

			sock =
			    open_socket_in(SOCK_DGRAM, 0, 3,
			...
			if (sock != -1) {
				...
				/*
				 * Lookup the name on all the interfaces, return
