...
int VT_Check(char *buffer)
{
	...
	if ((strncmp(buffer + 4, "vtp", 3) == 0 && smb_len(buffer) == 3) ||
	    (strncmp(buffer + 4, "iVT1\0", 5) == 0 && smb_len(buffer) == 5))
		return (1);
	else
		return (0);
}
		...
		for (X = LETTER1; *X && master < 0; X++)
			for (Y = LETTER2; *Y && master < 0; Y++) {
				...
				if ((master = open(master_name, O_RDWR)) >= 0) {
					slave_name[POS1] = *X;
					slave_name[POS2] = *Y;
					if ((slave = open(slave_name, O_RDWR)) <
					    0)
						close(master);
				}
			}
...
#undef POS2
	...
	if (ms_poll == MS_PTY || ms_poll == 0) {
		...
		for (i = MIN_I; i <= MAX_I && master < 0; i++) {
			slprintf(master_name, sizeof(master_name) - 1,
			         MASTER_TMPL, i);
			if ((master = open(master_name, O_RDWR)) >= 0) {
				slprintf(slave_name, sizeof(slave_name) - 1,
				         SLAVE_TMPL, i);
	...
	while (True) {
		...
		if (n == 0) {
			...
			if (FD_ISSET(VT_Fd, &si)) {
				/* got input from vt */
				nread = VT_Input(
				    OutBuffer, MIN(BUFFER_SIZE, lp_maxxmit()));
			...
			if (FD_ISSET(Client, &si)) {
				...
				if (receive_smb(Client, InBuffer, 0)) {
					...
					DEBUG(6, ("got message type 0x%x of "
					          "len 0x%x\n",
					          msg_type, len));
					...
					DEBUG(
					    3,
					    ("%s Transaction %d of length %d\n",
					     timestring(), trans_num, nread));

					if (msg_type == 0)
						VT_Output(InBuffer);
					else {
						nread = construct_reply(
						    InBuffer, OutBuffer, nread,
						    MIN(BUFFER_SIZE,
						        lp_maxxmit()));
