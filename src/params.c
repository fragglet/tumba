 ...
 */
{
	...
	while ((EOF != c) && (c > 0)) {
		...
		case '\r': /* Explicitly remove '\r' because the older */
			c = getc(InFile); /* version called fgets_slash() which
			                     also  */
			break; /* removes them.                            */
		...
		case '\n': /* Marks end of value unless there's a '\'. */
			...
			if (i < 0)
				...
				for (end = i; (end >= 0) && isspace(bufr[end]);
				     end--)
					;
				c = getc(InFile);
			}
		...
		default: /* All others verbatim.  Note that spaces do */
			bufr[i++] =
			    c; /* not advance <end>.  This allows trimming  */
			if (!isspace(
 ...
 */
{
	...
	if (NULL != bufr) /* If we already have a buffer */
		result = Parse(InFile, sfunc,
		               pfunc); /* (recursive call), then just */
	                               /* use it.                     */
