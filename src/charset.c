...
static void add_dos_char(int lower, BOOL map_lower_to_upper, int upper,
                         BOOL map_upper_to_lower)
{
	lower &= 0xff;
	upper &= 0xff;
	DEBUG(6, ("Adding chars 0x%x 0x%x (l->u = %s) (u->l = %s)\n", lower,
	          upper, map_lower_to_upper ? "True" : "False",
...
static codepage_p load_client_codepage(int client_codepage)
{
	...
	slprintf(&codepage_file_name[strlen(codepage_file_name)],
	         sizeof(pstring) - (strlen(codepage_file_name) + 1), "%03d",
	         client_codepage);
	...
	if (size < CODEPAGE_HEADER_SIZE ||
	    size > (CODEPAGE_HEADER_SIZE + 4 * MAXCODEPAGELINES)) {
		DEBUG(
		    0,
		    ("load_client_codepage: file %s is an incorrect size for a \
code page file.\n",
		     codepage_file_name));
		return NULL;
	}
	...
	if ((fp = fopen(codepage_file_name, "r")) == NULL) {
		DEBUG(0, ("load_client_codepage: cannot open file %s. Error "
		          "was %s\n",
	...
	if (fread(buf, 1, CODEPAGE_HEADER_SIZE, fp) != CODEPAGE_HEADER_SIZE) {
		DEBUG(0, ("load_client_codepage: cannot read header from file "
		          "%s. Error was %s\n",
		          codepage_file_name, strerror(errno)));
		goto clean_and_exit;
	}
	...
	if (SVAL(buf, CODEPAGE_VERSION_OFFSET) != CODEPAGE_FILE_VERSION_ID) {
		DEBUG(
		    0,
		    ("load_client_codepage: filename %s has incorrect version id. \
Needed %hu, got %hu.\n",
	...
	if (SVAL(buf, CODEPAGE_CLIENT_CODEPAGE_OFFSET) !=
	    (uint16) client_codepage) {
		DEBUG(
		    0,
		    ("load_client_codepage: filename %s has incorrect codepage. \
Needed %hu, got %hu.\n",
	...
	if (IVAL(buf, CODEPAGE_LENGTH_OFFSET) !=
		...
		DEBUG(
		    0,
		    ("load_client_codepage: filename %s has incorrect size headers. \
Needed %u, got %u.\n",
	...
	if ((size % 4) != 0) {
		DEBUG(
		    0,
		    ("load_client_codepage: filename %s has a codepage size not a \
multiple of 4.\n",
		     codepage_file_name));
		goto clean_and_exit;
	}
	...
	if (fread((char *) cp_p, 1, size, fp) != size) {
		DEBUG(0, ("load_client_codepage: read fail on file %s. Error "
		          "was %s.\n",
		          codepage_file_name, strerror(errno)));
		goto clean_and_exit;
	}
...
void codepage_initialise(int client_codepage)
{
	...
	if (cp != NULL) {
		DEBUG(6, ("codepage_initialise: called twice - ignoring second "
		          "client code page = %d\n",
		          client_codepage));
		return;
	}
...
void add_char_string(char *s)
{
	...
	for (t = strtok(extra_chars, " \t\r\n"); t;
		...
		if (isdigit((unsigned char) *t) || (*t) == '-') {
			...
			sscanf(t, "%c:%c", &c1, &c2);
			add_dos_char((unsigned char) c1, True,
			             (unsigned char) c2, True);
		}
	}
