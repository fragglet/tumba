...
struct pack_desc {
	...
	int usedlen;     /* total used size (usedlen <= neededlen and usedlen <=
	                    buflen) */
...
static int package(va_alist) va_dcl
{
	...
	case 'B': /* byte (with optional counter) */
		needed = get_counter(&p->curpos);
		{
			...
			if (p->buflen >= needed)
				StrnCpy(p->structbuf, s ? s : "", needed);
		}
		break;
	case 'z': /* offset to zero terminated string (4 byte) */
		str = va_arg(args, char *);
                                ...
                                char **rparam, int *rdata_len, int *rparam_len)
{
	...
	*rdata = REALLOC(*rdata, *rdata_len);
	...
	if (uLevel >= 10) {
		...
		SIVAL(p, usri11_usr_comment,
		      PTR_DIFF(p2, p)); /* user_comment */
		pstrcpy(p2, "UserComment");
		p2 = skip_string(p2, 1);

		/* EEK! the cifsrap.txt doesn't have this in!!!! */
		SIVAL(p, usri11_full_name, PTR_DIFF(p2, p)); /* full name */
		pstrcpy(p2, ((vuser != NULL) ? vuser->real_name
		                             : UserName)); /* simeon */
		p2 = skip_string(p2, 1);
	}
	...
	{
		SSVAL(p, usri11_priv,
		      Connections[cnum].admin_user ? USER_PRIV_ADMIN
		                                   : USER_PRIV_USER);
		...
		SIVAL(p, usri11_logon_server,
		      PTR_DIFF(p2, p)); /* logon server */
		pstrcpy(p2, "\\\\*");
		p2 = skip_string(p2, 1);
		SSVAL(p, usri11_country_code, 0); /* country code */

		SIVAL(p, usri11_workstations,
		      PTR_DIFF(p2, p)); /* workstations */
		pstrcpy(p2, "");
		p2 = skip_string(p2, 1);
...
/****************************************************************************
  ...
  ****************************************************************************/
static BOOL api_WAccessGetUserPerms(int cnum, uint16 vuid, char *param,
                                    char *data, int mdrcnt, int mprcnt,
                                    char **rdata, char **rparam, int *rdata_len,
                                    int *rparam_len)
{
