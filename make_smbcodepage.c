...
int do_compile(int codepage, char *input_file, char *output_file)
{
	...
	if (stat((char *) input_file, &st) != 0) {
		fprintf(stderr,
		        "%s: failed to get the file size for file %s. Error "
		        "was %s\n",
...
The maximum size I will believe is 100k.\n",
	...
	if (num_lines > MAXCODEPAGELINES) {
		fprintf(
		    stderr,
		    "%s: There can be a maximum %d lines of data in a codepage \
definition file. File %s has %d.\n",
	...
	for (i = 0; i < num_lines; i++) {
		...
		if (!parse_bool(token_buf, &b))
			parse_error(
			    buf, "fourth value doesn't resolve to a boolean");
...
code page file.\n",
	...
	if (fread(header_buf, 1, CODEPAGE_HEADER_SIZE, fp) !=
